
<div class="container">
		<a href="<?php echo base_url().'index.php/requests/pendingrequests'?>" title="back to Submissions History">&laquo; Back</a>
        <h1 class="row">
        	<span class="col-sm-offset-1 col-sm-8">Request Details</span>
            <span class="h3 col-sm-3"><span class="glyphicon glyphicon-time" style="color:#39b3d7"></span>  Pending</span>
        </h1>
        <hr>
        <form class="form-horizontal" role="form">
        <h2 class="col-sm-offset-1">Request Information:</h2>
        <div class="row">
                <div class="col-sm-offset-1 col-sm-5">
                    <address>
                      <strong>Requested By:</strong><br>
                      Victoria Violet
                    </address>
                </div>	
                <div class="col-sm-4">
                <address>
                      <strong>Request Type:</strong><br>
                      Link account to provider
                    </address>
                 </div>	
            </div>
             <div class="row">
                <div class="col-sm-offset-1 col-sm-5">
                <address>
                      <strong>Email:</strong><br>
                      victoriaviolet@serviceprovider.gov
                    </address>
                 </div>	
                 <div class="col-sm-4">
                    <address>
                      <strong>Phone #:</strong><br>
                     (333)-333-3333
                    </address>
                </div>	
            </div>
            <div class="row h4">
            <div class="col-xs-5">
            	<hr>
            </div>         	
            	<span class="label label-primary label-lg col-xs-2" style="margin: 10px 0px;"><span class="glyphicon glyphicon-link"></span> Link</span>
            <div class="col-xs-5">
            	<hr>
            </div>
            </div>
            <h2 class="col-sm-offset-1">Provider Information:</h2>
			<div class="row">
                    <div class="col-sm-offset-1 col-sm-5">
                        <address>
                          <strong>Provider Name:</strong><br>
                          Food Bank for Families
                        </address>
                    </div>	
                    <div class="col-sm-4">
                        <address>
                          <strong>Provider ID:</strong><br>
                          22222200
                        </address>
                     </div>	
                </div>
                <div class="row">
                    <div class="col-sm-offset-1 col-sm-5">
                        <address>
                          <strong>Program Type:</strong><br>
                          Food Bank
                        </address>
                    </div>	
                    <div class="col-sm-4">
                        <address>
                          <strong>Address:</strong><br>
                          123 Main Street<br>
                          Center City, MS 30000<br>
                        </address>
                     </div>	
                </div>
                 <div class="row">
                    <div class="col-sm-offset-1 col-sm-5">
                        <address>
                          <strong>Telephone:</strong><br>
                          <abbr title="Toll Free">T:</abbr> (123) 456-7890<br>
                        </address>
                    </div>                        
                    <div class="col-sm-4">
                        <address>
                          <strong>Website:</strong><br>
                          <a href="mailto:#">www.serviceprovider.com</a>
                        </address>
                     </div>	
                </div>                
            </form>
	</div>
    <hr>
    <a href="<?php echo base_url().'index.php/submissions/submissionshistory'?>" title="back to Submissions History">&laquo; Back</a>